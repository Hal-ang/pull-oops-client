...
const Main = () => {
  ...
  return (
    <div className="h-screen w-full">
      <div id="main_header" className="fixed w-full flex">
        ...
        <div
          style={{top: 72}}
          className="h-8 w-full bg-gradient-to-b from-white to-transparent absolute"></div>
      </div>
      <div style={{height: 72}} className="flex-1"></div>
      <div className="w-full px-6 h-full overflow-y-scroll snap-y pt-10 pb-10">
        {steps.map((step, index) => (
          <div
            style={{height: cardHeight}}
            key={index}
            className="w-full bg-black mb-8 rounded-md  snap-center"></div>
        ))}
      </div>
    </div>
  );
